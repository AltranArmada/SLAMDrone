...
void Planner::setStart(const geometry_msgs::PoseWithCovarianceStamped::ConstPtr& initial) {
  ...
  ROS_ERROR("I am seeing a new start x: %f y: %f t:%f height: %d width %d", x, y, Helper::toDeg(t), grid->info.height, grid->info.width);
  ...
  if (grid->info.height >= y && y >= 0 && grid->info.width >= x && x >= 0) {
    ...
    ROS_ERROR("Invalid start x: %f y: %f t:%f", x, y, Helper::toDeg(t));
  }
}
...
void Planner::setGoal(const geometry_msgs::PoseStamped::ConstPtr& end) {
  ...
  ROS_ERROR("I am seeing a new goal x: %f y: %f t:%f", x, y, Helper::toDeg(t));
  ...
  if (grid->info.height >= y && y >= 0 && grid->info.width >= x && x >= 0) {
    ...
    ROS_ERROR("Invalid goal x: %f y: %f t:%f", x, y, Helper::toDeg(t));
  }
}
...
void Planner::plan() {
  ...
  if (validStart && validGoal) {
    ...
    ROS_ERROR("missing goal or start");
  }
}
